---
doc_type: PRD
fragment_id: prd-sdlc-visualizer-002
title: 02 - 功能需求
version: 2.0-patch2
...
- fragment_id: prd-sdlc-visualizer-feat11-629
  version: 1.0.0
- fragment_id: prd-sdlc-visualizer-feat12-629
  version: 1.0.0
...
## 1. 系统功能架构图 {#sec-1-xitonggongnengjiagoutu}
```mermaid
flowchart TD
...
## 2. 模块-功能点树状图 {#sec-2-mokuaigongnengu70b9u6811zhuang}
...
## 3. 端到端用户旅程 {#sec-3-u7aefdaou7aefyonghuu65c5u7a0b}
### 3.1 主旅程（Happy Path） {#sec-31-u4e3bu65c5u7a0bhappy-path}
...
### 3.2 关键替代路径 {#sec-32-guanu952eu66ffdailujing}
...
### 3.3 异常退出路径 {#sec-33-yichangtuichulujing}
...
## 4. 角色职责描述 {#sec-4-jiaoseu804cu8d23u63cfu8ff0}
...
## 5. 全局业务规则 {#sec-5-quanu5c40yewuguize}
...
## 6. 状态机 {#sec-6-zhuangtaiji}
### 6.1 项目级状态机（Project） {#sec-61-u9879mujizhuangtaijiproject}
```mermaid
stateDiagram-v2
    [*] --> Draft : 创建项目
...
### 6.2 Skill 级状态机（含 PocketFlow 三阶段） {#sec-62-skill-jizhuangtaijiu542b-pock}
```mermaid
stateDiagram-v2
    [*] --> NOT_STARTED : 初始化
...
### 6.3 产物状态机（Artifact） {#sec-63-chanu7269zhuangtaijiartifact}
```mermaid
stateDiagram-v2
...
## 7. 详细需求清单：模块映射目录 {#sec-7-xiangu7ec6xuqiuu6e05danmokuaiu}
...
## 6. 状态机定义（业务视角） {#sec-6-zhuangtaijiu5b9au4e49yewuu89c6}
### 6.1 变更（Change/Project）状态机 {#sec-61-biangengchangeprojectzhuangta}
```mermaid
stateDiagram-v2
    [*] --> Draft : 创建项目
...
### 6.2 Stage 状态机（含审查迭代） {#sec-62-stage-zhuangtaijiu542bshencha}
| 当前状态 | 允许转移 | 触发事件 |
|----------|----------|----------|
...
### 6.3 异常状态转移 {#sec-63-yichangzhuangtaizhuanyi}
...
## 7. 详细 PRD 清单（目录映射） {#sec-7-xiangu7ec6-prd-u6e05danmuluu66}
...
## 1. 功能结构 {#sec-1-gongnengjiegou}
### 1.1 模块-功能点树状图 {#sec-11-mokuaigongnengu70b9u6811zhuan}
```text
arsitect 自适应架构引擎
|
...
### 1.2 用户故事地图 {#sec-12-yonghuguu4e8bditu}
| 旅程阶段 | 涉及模块 | 用户故事 |
|---------|---------|---------|
...
## 2. 端到端用户旅程 {#sec-2-u7aefdaou7aefyonghuu65c5u7a0b}
### 2.1 旅程一：快速修复（Trivial 路径） {#sec-21-u65c5u7a0byiu5febsuxiufutrivi}
**目标用户**：Alex（后端开发者）
...
### 2.2 旅程二：新模块开发（Deep 路径） {#sec-22-u65c5u7a0berxinmokuaikaifadee}
**目标用户**：Ben（Tech Lead）
...
## 3. 角色职责描述 {#sec-3-jiaoseu804cu8d23u63cfu8ff0}
| 角色 | 职责 | 权限范围 | 禁止行为 |
|------|------|---------|---------|
...
## 4. 状态机（业务视角） {#sec-4-zhuangtaijiyewuu89c6jiao}
### 4.1 需求生命周期状态机 {#sec-41-xuqiushengu547dzhouqizhuangta}
```mermaid
stateDiagram-v2
    [*] --> Draft: 提交需求
...
### 4.2 Gate 状态机 {#sec-42-gate-zhuangtaiji}
```mermaid
stateDiagram-v2
    [*] --> Pending: 阶段完成
...
## 6. 端到端流程图 {#sec-6-u7aefdaou7aefliuu7a0btu}
### 6.1 复杂度路由与路径执行流程 {#sec-61-fuu6742duluyouyulujingzhixing}
```mermaid
flowchart TD
...
### 6.2 架构漂移检测流程 {#sec-62-jiagouu6f02yijianceliuu7a0b}
```mermaid
flowchart TD
...
## 7. 详细需求清单：模块映射 {#sec-7-xiangu7ec6xuqiuu6e05danmokuaiu_1}
| 编号 | 模块名称 | 对应目录 | 状态 |
|------|---------|---------|------|
