...
title: DR-009 模块需求规格书 — 模板引擎（Template Engine）
version: '1.0'
...
### 2.1 页面 / 入口清单 {#sec-21-yeu9762-rukouu6e05dan}
...
### 3.4 接口响应字段（模块间契约，非 API 规格） {#sec-34-jiekouu54cdyingu5b57u6bb5moku}
| 字段名 | 说明 | 消费者模块 |
|:-------|:-----|:-----------|
...
### 5.1 页面：Pg_TemplateSelect（模板选择面板） {#sec-51-yeu9762pgtemplateselectmobanx}
...
### 5.2 页面：Pg_ComplexityRouter（复杂度路由面板） {#sec-52-yeu9762pgcomplexityrouterfuu6}
...
### 5.3 页面：Pg_DeviationConfirm（偏离确认弹窗） {#sec-53-yeu9762pgdeviationconfirmu504}
...
### 5.4 页面：Pg_StageDefinition（Stage 定义管理面板） {#sec-54-yeu9762pgstagedefinitionstage}
...
### 5.5 页面：Pg_DeviationLog（偏离记录面板） {#sec-55-yeu9762pgdeviationlogu504fu79}
