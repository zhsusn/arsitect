---
doc_type: "CHANGELOG"
fragment_id: "changelog-sdlc-visualizer-167"
title: "AI Architecture Decision - sdlc-visualizer"
version: "1.0"
...
## 1. 模块级原语映射 {#sec-1-mokuaijiu539fu8bedu6620u5c04}
...
### 1.1 关键结论 {#sec-11-guanu952ejieu8bba}
...
## 2. 架构模式选择 {#sec-2-jiagoumou5f0fxuanu62e9}
...
## 3. 上下文管理策略 {#sec-3-shangxiawenguanliceu7565}
### 3.1 三级上下文模型 {#sec-31-sanjishangxiawenmoxing}
...
### 3.2 渐进式披露方案 {#sec-32-u6e10jinu5f0fu62abu9732u65b9u}
...
### 3.3 上下文压缩与重置规则 {#sec-33-shangxiawenyasuoyuchongzhigui}
...
## 4. 安全边界 {#sec-4-anquanu8fb9u754c}
### 4.1 工具权限白名单 {#sec-41-gongu5177quanxianu767dmingdan}
| 组件 | 允许操作 | 禁止操作 | 说明 |
|------|----------|----------|------|
...
### 4.2 敏感操作确认机制 {#sec-42-u654fu611fu64cdu4f5cquerenjiz}
...
## 5. 与 rollback-plan.md 的衔接 {#sec-5-yu-rollbackplanmd-deu8854jie}
### 5.1 AI 模型错误触发回滚的条件 {#sec-51-ai-moxingu9519u8befu89e6fahui}
| 错误场景 | 检测方式 | 回滚目标 | 回滚粒度 |
|----------|----------|----------|----------|
...
### 5.2 回滚实现机制 {#sec-52-huigunshixianjizhi}
...
### 5.3 AI 回滚与人工回滚的协作 {#sec-53-ai-huigunyurengonghuigundeu53}
...
## 模块级原语映射 {#sec-mokuaijiu539fu8bedu6620u5c04}
...
## 架构模式选择 {#sec-jiagoumou5f0fxuanu62e9}
...
## 上下文管理策略 {#sec-shangxiawenguanliceu7565}
...
## 与 rollback-plan.md 的衔接 {#sec-yu-rollbackplanmd-deu8854jie}
...
## 原语选型总览 {#sec-u539fu8bedxuanxingzonglan}
本项目涉及大量 AI 调用（LLM 需求解析、架构生成、代码生成、审查、验证），需为每个模块选择合适的 AI 原语。

### 1. ComplexityRouter（复杂度路由） {#sec-1-complexityrouterfuu6742duluyou}
| 维度 | 评分 (1-10) | 权重 | 加权分 |
|------|-------------|------|--------|
| 复杂度 | 5 | 25% | 1.25 |
| 复用性 | 8 | 20% | 1.60 |
...
### 2. C4ModelManager（C4 DSL 管理） {#sec-2-c4modelmanagerc4-dsl-guanli}
| 维度 | 评分 (1-10) | 权重 | 加权分 |
|------|-------------|------|--------|
| 复杂度 | 8 | 25% | 2.00 |
| 复用性 | 6 | 20% | 1.20 |
...
### 3. DriftCollector（架构漂移检测） {#sec-3-driftcollectorjiagouu6f02yijia}
| 维度 | 评分 (1-10) | 权重 | 加权分 |
|------|-------------|------|--------|
| 复杂度 | 8 | 25% | 2.00 |
| 复用性 | 5 | 20% | 1.00 |
...
### 4. PrototypeVerifier（原型验证 + DSL 回写） {#sec-4-prototypeverifieru539fxingyanu}
| 维度 | 评分 (1-10) | 权重 | 加权分 |
|------|-------------|------|--------|
| 复杂度 | 7 | 25% | 1.75 |
| 复用性 | 4 | 20% | 0.80 |
...
### 5. OpenHandsExecutor（沙箱执行器封装） {#sec-5-openhandsexecutoru6c99u7bb1zhi}
| 维度 | 评分 (1-10) | 权重 | 加权分 |
|------|-------------|------|--------|
| 复杂度 | 6 | 25% | 1.50 |
| 复用性 | 7 | 20% | 1.40 |
...
### 6. StageGateController（门控流转） {#sec-6-stagegatecontrollermenkongliuz}
| 维度 | 评分 (1-10) | 权重 | 加权分 |
|------|-------------|------|--------|
| 复杂度 | 5 | 25% | 1.25 |
| 复用性 | 9 | 20% | 1.80 |
...
### 渐进式披露方案 {#sec-u6e10jinu5f0fu62abu9732u65b9u684}
...
### 上下文压缩/重置规则 {#sec-shangxiawenyasuochongzhiguize}
...
### 工具权限白名单 {#sec-gongu5177quanxianu767dmingdan}
...
### 敏感操作确认机制 {#sec-u654fu611fu64cdu4f5cquerenjizhi}
...
## 回滚衔接 {#sec-huigunu8854jie}
### AI 模型错误触发回滚的条件 {#sec-ai-moxingu9519u8befu89e6fahuigun}
...
### 回滚方案索引 {#sec-huigunu65b9u6848suoyin}
