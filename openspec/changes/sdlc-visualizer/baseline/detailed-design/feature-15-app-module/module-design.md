...
title: DR-015 Application 与模块治理 — 模块详细设计
version: 1.0.0
...
> - `@C4-L3-Component:applicationrepository`
> - `@C4-L3-Component:applicationservice`
> - `@C4-L3-Component:coststatsrepository`
> - `@C4-L3-Component:coststatsservice`
> - `@C4-L3-Component:db-repository`
> - `@C4-L3-Component:file-repository`
> - `@C4-L3-Component:filesystemadapter`
> - `@C4-L3-Component:modulerepository`
> - `@C4-L3-Component:moduleservice`
> - `@C4-L3-Component:projectrepository`
...
flowchart TB
    subgraph API_Layer["API 层"]
        ...
        R_Dependency["/api/v1/projects/{id}/module-dependencies<br>ModuleDependencyRouter"]
    end
    ...
    subgraph Service_Layer["Service 层"]
        ...
        S_Dependency["ModuleDependencyService<br>契约检测 / 状态更新 / Stale 传播"]
    end
    ...
    subgraph Repository_Layer["Repository 层"]
        ...
        Repo_Dependency["ModuleDependencyRepository"]
    end
    ...
    R_Module --> S_Module
    R_Stats --> S_Stats
    R_Dependency --> S_Dependency

    S_App --> Repo_App
    S_Module --> Repo_Module
    S_Stats --> Repo_Stats
    S_Dependency --> Repo_Dependency

    S_App --> Ext_Project
    S_Stats --> Ext_SkillExec
    S_Dependency --> Ext_Artifact
    S_Module --> Ext_Flow
```
...
### 4.1 Application 状态机 {#sec-41-application-zhuangtaiji}
```mermaid
stateDiagram-v2
    [*] --> ACTIVE: 创建成功
...
### 4.2 Module 里程碑状态机（P1） {#sec-42-module-liu7a0bu7891zhuangtaij}
```mermaid
stateDiagram-v2
...
### 4.3 跨模块契约依赖状态机（P1） {#sec-43-u8de8mokuaiqiyueyiu8d56zhuang}
```mermaid
stateDiagram-v2
    [*] --> PENDING: 依赖声明创建
...
| UT-007 | AC-13 (P1) | Given 3 个 Module 全部 Stage PASSED，When `update_module_milestone()`，Then milestone_status = COMPLETED | Mock `ProjectStageRepository` 返回全部 PASSED |
