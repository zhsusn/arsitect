...
fragment_id: detail-design-sdlc-visualizer-shared-322
title: shared/design.md — 公共设计组件与规范
version: 1.0.0
...
> - `@C4-L3-Component:archvalidationstore`
> - `@C4-L3-Component:artifactstore`
> - `@C4-L3-Component:file-repository`
> - `@C4-L3-Component:filesystemadapter`
> - `@C4-L3-Component:gatestore`
> - `@C4-L3-Component:historystore`
> - `@C4-L3-Component:localfilesystemadapter`
> - `@C4-L3-Component:monitoringstore`
> - `@C4-L3-Component:projectstore`
> - `@C4-L3-Component:stagestore`

> **说明**：本文件定义跨模块复用的设计组件、抽象接口和编码规范。模块级设计中对这些组件的引用通过本文件实现。

---

## 1. 分页 DTO {#sec-1-fenye-dto}
### 1.1 PageRequestDTO {#sec-11-pagerequestdto}
所有分页查询的请求参数基类。
...
### 1.2 PageResponseDTO {#sec-12-pageresponsedto}
所有分页查询的响应结构基类。
...
## 2. 全局异常处理基类 {#sec-2-quanu5c40yichangchuliu57falei}
### 2.1 异常类层次 {#sec-21-yichangleiu5c42u6b21}
```
...
### 2.2 基类定义（Python 后端） {#sec-22-u57faleiu5b9au4e49python-u540}
```python
...
### 2.3 全局异常处理器（FastAPI） {#sec-23-quanu5c40yichangchuliqifastap}
```python
...
## 3. 文件系统适配器接口 {#sec-3-wenjianxitongshipeiqijiekou}
### 3.1 抽象接口 {#sec-31-u62bdu8c61jiekou}
```python
...
### 3.2 本地文件系统实现（MVP） {#sec-32-bendiwenjianxitongshixianmvp}
```python
...
## 4. Zustand Store 模式规范 {#sec-4-zustand-store-mou5f0fguifan}
### 4.1 Store 命名规范 {#sec-41-store-u547dmingguifan}
| 模块 | Store 名称 | 状态范围 |
|------|-----------|----------|
...
### 4.2 Store 结构模板 {#sec-42-store-jiegoumoban}
```typescript
...
### 4.3 Store 拆分原则 {#sec-43-store-u62c6fenu539fze}
...
## 5. 前端公共组件规范 {#sec-5-u524du7aefu516cgongzujianguifa}
### 5.1 侧滑面板（Drawer）规范 {#sec-51-u4fa7u6ed1u9762bandrawerguifa}
所有侧滑面板统一遵循以下规范：
...
### 5.2 弹窗（Modal）规范 {#sec-52-tanchuangmodalguifan}
| 属性 | 默认值 | 说明 |
|------|--------|------|
...
### 5.3 骨架屏（Skeleton）规范 {#sec-53-u9aa8jiau5c4fskeletonguifan}
...
## 6. 编码规范 {#sec-6-bianmaguifan}
### 6.1 Python（后端） {#sec-61-pythonu540eu7aef}
- 遵循 `python-google-style` Skill 规范
- 行宽 100 字符
- 类型注解必须（mypy strict 模式）
- Google Style docstring

### 6.2 TypeScript / React（前端） {#sec-62-typescript-reactu524du7aef}
- 函数组件优先，Hooks 管理状态
