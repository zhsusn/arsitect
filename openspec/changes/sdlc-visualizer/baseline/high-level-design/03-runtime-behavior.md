...
## 1. 全局状态机 {#sec-1-quanu5c40zhuangtaiji}
### 1.1 项目级状态机（Project） {#sec-11-u9879mujizhuangtaijiproject}
```mermaid
stateDiagram-v2
    [*] --> Draft : 创建项目
...
### 1.2 Skill 级状态机（含 PocketFlow 三阶段） {#sec-12-skill-jizhuangtaijiu542b-pock}
```mermaid
stateDiagram-v2
    [*] --> NOT_STARTED : 初始化
...
### 1.3 产物状态机（Artifact） {#sec-13-chanu7269zhuangtaijiartifact}
```mermaid
stateDiagram-v2
...
## 2. 关键流程时序图 {#sec-2-guanu952eliuu7a0bshixutu}
### 2.1 Skill 执行主链路 {#sec-21-skill-zhixingu4e3bu94felu}
```mermaid
sequenceDiagram
    actor U as 用户
...
### 2.2 Gate 审批链路 {#sec-22-gate-shenpiu94felu}
```mermaid
sequenceDiagram
    actor U as 用户
    participant FE as React SPA
...
### 2.3 产物审查与重新生成链路 {#sec-23-chanu7269shenchayuchongxinshe}
```mermaid
sequenceDiagram
    actor U as 用户
    participant FE as React SPA
...
### 2.4 C4 DSL 生成与穿透链路 {#sec-24-c4-dsl-shengu6210yuu7a7fu900f}
```mermaid
sequenceDiagram
    actor U as 用户
    participant FE as React SPA
...
## 3. 异常处理全局 {#sec-3-yichangchuliquanu5c40}
### 3.1 错误分类 {#sec-31-u9519u8beffenlei}
| 类别 | 定义 | 典型场景 | 影响范围 |
|------|------|----------|----------|
...
### 3.3 重试策略 {#sec-33-chongshiceu7565}
| 场景 | 重试次数 | 退避策略 | 死信处理 |
|------|----------|----------|----------|
...
### 3.4 与回滚方案的衔接 {#sec-34-yuhuigunu65b9u6848deu8854jie}
...
## 4. 算法选型（AI 项目） {#sec-4-suanu6cd5xuanxingai-u9879mu}
### 4.1 模型基座选择 {#sec-41-moxingu57fau5ea7xuanu62e9}
| 功能 | 基座 | 调用方式 | 选型理由 |
|------|------|----------|----------|
...
### 4.2 关键自研算法 {#sec-42-guanu952eziu7814suanu6cd5}
#### 算法 A：复杂度路由规则引擎
...
### 4.3 与其他模块的耦合方式 {#sec-43-yuqitamokuaideu8026u5408u65b9}
| 算法 | 上游依赖 | 下游消费 | 耦合强度 |
|------|----------|----------|----------|
...
### 1.1 跨模块核心实体状态流转 {#sec-11-u8de8mokuaihexinshitizhuangta}
...
### 1.2 状态流转与 Gate 联动规则 {#sec-12-zhuangtailiuzhuanyu-gate-u805}
...
### 2.1 Skill 执行完整链路 {#sec-21-skill-zhixingwanu6574u94felu}
...
### 2.3 范围锚定与 Stale 传播链路 {#sec-23-fanweiu951au5b9ayu-stale-chua}
...
## 4. 算法选型（AI 项目必选） {#sec-4-suanu6cd5xuanxingai-u9879muu5f}
### 4.1 Gate 自检摘要生成策略 {#sec-41-gate-zijianzhaiyaoshengu6210c}
...
### 4.2 规模评估 Triage 算法 {#sec-42-guimopinggu-triage-suanu6cd5}
...
### 4.3 Stale 影响分析引擎 {#sec-43-stale-u5f71u54cdfenxiyinu64ce}
...
## 5. 需求可追溯性 {#sec-5-xuqiuu53efzhuiu6eafxing}
| 需求编号 | 需求描述 | 本文件对应章节 | 验证方式 |
|---------|---------|-------------|---------|
