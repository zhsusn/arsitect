...
## 1. 数据架构 {#sec-1-shujujiagou}
### 1.1 逻辑 ER 图 {#sec-11-luoji-er-tu}
```mermaid
erDiagram
...
### 1.2 主数据流向 {#sec-12-u4e3bshujuliuxiang}
```mermaid
flowchart LR
    subgraph Source["数据源"]
...
### 1.3 存储策略 {#sec-13-cunu50a8ceu7565}
| 数据类型 | 存储介质 | 策略 | 理由 |
|----------|----------|------|------|
...
### 1.4 核心表清单（无字段类型） {#sec-14-hexinbiaou6e05danwuu5b57u6bb5}
| 表名 | 职责 | 关联表 | 数据量级（MVP） |
|------|------|--------|----------------|
...
## 2. 接口契约 {#sec-2-jiekouqiyue}
### 2.1 通信模式 {#sec-21-tongxinmou5f0f}
| 通信场景 | 模式 | 协议 | 理由 |
|----------|------|------|------|
...
### 2.2 数据契约原则 {#sec-22-shujuqiyueu539fze}
| 原则 | 说明 |
|------|------|
...
### 2.3 前后端接口分组 {#sec-23-u524du540eu7aefjiekoufenzu}
| 接口组 | 职责 | 关键资源 |
|--------|------|----------|
...
## 3. 模块职责 {#sec-3-mokuaiu804cu8d23}
### 3.1 各模块输入/输出/核心职责/对外依赖 {#sec-31-u5404mokuaiu8f93ruu8f93chuhex}
| 模块 | 输入 | 输出 | 核心职责 | 对外依赖 |
|------|------|------|----------|----------|
...
### 3.2 跨模块数据交互矩阵 {#sec-32-u8de8mokuaishujujiaou4e92u77e}
```mermaid
flowchart LR
    subgraph Canvas["画布层"]
        Pg_Canvas["SDLC 画布"]
...
### 1.3 存储策略（SQLite / 文件系统分离） {#sec-13-cunu50a8ceu7565sqlite-wenjian}
...
### 1.4 核心表清单 {#sec-14-hexinbiaou6e05dan}
| 表名 | 职责 |
|------|------|
...
### 2.1 通信模式（REST + WebSocket） {#sec-21-tongxinmou5f0frest-websocket}
...
### 2.2 数据契约与版本策略 {#sec-22-shujuqiyueyubanbenceu7565}
