...
> **模块编号**：DR-015
> **模块名称**：Application 与模块治理
> **优先级**：P0（Application CRUD）/ P1（Module 级里程碑）
> **关联需求**：REQ-P0-001（Application 创建/选择）、Module 级里程碑（P1）
> **关联用户故事**：US-001（创建项目时选择/创建 Application）
> **关联业务规则**：BR-015、BR-018、BR-019
> **版本**：v1.0
> **状态**：Draft
...
### 5.1 页面：Pg_AppList / Application 列表页
...
### 5.2 页面：Pg_AppCreate / 创建 Application 弹窗
...
### 5.3 页面：Pg_AppDetail / Application 详情页
...
### 5.4 页面：Pg_ProjectCreate_AppSelect / 项目创建-Application 选择步骤
...
### 5.5 页面：Pg_ModuleMilestone (P1) / Module 里程碑视图
...
### 5.6 页面：Pg_ModuleDependency (P1) / 跨模块契约依赖面板
