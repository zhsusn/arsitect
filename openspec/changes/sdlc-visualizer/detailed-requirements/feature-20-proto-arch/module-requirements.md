...
> **模块编号**：DR-020  
> **模块名称**：原型-架构双向绑定  
> **关联需求**：接口缺失检测、C4 DSL 自动回写  
> **关联用户故事**：US-016（Wireframe 中发现接口缺失时回写）  
> **版本**：v1.0
> **优先级**：P1
> **状态**：Active → Implemented（v1.1）
...
3. 评审状态以标签形式回显在列表行中
> **MVP 说明**：当前版本未实现独立的 architecture_change 表和 C4 DSL 文件级回写，评审结果以 `coverage_scan_item.review_status` 字段承载。P1 阶段将扩展为独立的架构变更记录表和 YAML 补丁机制。
...
### 5.1 Pg_CoverageDashboard / 接口覆盖度仪表盘
...
> **MVP 说明**：独立的架构变更评审页（Pg_ArchChangeReview）未实现，评审功能以行级按钮形式内嵌在 Pg_BindingPanel 的 Gap Tab 中。P1 阶段将扩展为独立的变更评审页面，支持评论、Diff 视图、批量评审。
...
| 版本 | 日期 | 修改人 | 修改内容 |
|------|------|--------|----------|
| v1.0 | 2026-06-01 | AI Product Manager | 初始版本，基于 PRD 关键信息生成 DR-020 模块级详细需求 |
