...
> **版本**：v1.0
> **设计状态**：FROZEN
> **上游追溯**：DR-014 详细需求（REQ-P1-007/008）
> **下游消费**：无（本模块为数据消费终端）
> **变更**：sdlc-visualizer
...
> **设计说明**：
> - `UNIQUE(project_id, user_id)` 确保同一用户在项目中仅有一个角色。
> - 变更角色时更新 `updated_at`，原记录保留（不删除重写），便于审计追溯。
> - MVP 阶段用户系统为本地单机，user_id 与 username 直接存储于本表；
>   P1 支持多用户后，user_id 外键关联独立用户表。
...
> **设计说明**：
> - `action_type` 严格限定为 `openspec/config.yaml` 中定义的"关键操作"类型（BR-008）。
> - `details` 字段以 JSON 存储详细变更信息（如权限变更的前后角色），便于前端展示。
> - `is_sensitive=true` 的日志在 Developer 角色查看时进行脱敏处理（AC-S-003）。
> - 表设计为只追加（append-only），禁止 UPDATE/DELETE（BR-007）。
> - 日志量过大时（>10,000 条），查询自动截断至最近 10,000 条（EX-006）。

### 3.3 `token_consumption_records` — Token 消耗记录表
...
### 3.4 `monitoring_refresh_configs` — 看板刷新配置表
