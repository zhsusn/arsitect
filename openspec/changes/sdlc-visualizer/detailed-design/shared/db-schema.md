...
> - `path_accessible` 为运行时检测状态，系统定期（每 5 分钟）或按需检测路径可访问性。
...
## 7. template_stages — 模板 Stage 定义表
...
## 8. project_stages — 项目级 Stage 实例表
...
## 9. stage_review_status — Stage 审查状态表
...
> **设计说明**：
> - DR-004 在 Gate 裁决通过后更新 `current_status` 为 `PASSED` 或 `REVISION_REQUESTED`，
>   这是跨模块写操作，需通过 REST 接口完成，不直接操作对方数据库表。
> - `previous_status` 用于重新生成失败时状态回退。

---

## 10. gate_decisions — Gate 决策记录表
...
## 11. gate_decision_history — Gate 决策历史明细表
...
> **设计说明**：
> - `stale_flag` 标记上游基线变更后的产物过时状态（BR-019）。
> - `file_path` 在项目内唯一，确保同一项目下无重复路径。

---

## 13. artifact_versions — 产物版本记录表
...
> **设计说明**：
> - 每产物最多保留 10 条版本记录（BR-020），超出时最旧版本标记为归档。
> - `snapshot_id` 关联 Git 快照，支持版本回滚到具体提交。

---

## 14. c4_dsl_store — C4 DSL 存储表
