...
> **模块编号**：DR-009  
> **模块名称**：模板引擎（Template Engine）  
> **版本**：v1.0  
> **状态**：FROZEN  
> **设计日期**：2026-06-02  
> **上游基线**：PRD-000 v2.0-patch2 / HLD-001~003 / DR-009 详细需求
...
> **设计说明**：
> - MVP 阶段模板为系统预置，用户不可增删改。`config_json` 存储完整的模板配置，便于版本管理和快速读取。
> - P1 阶段如需支持自定义模板，可将 `templates` 扩展为用户可编辑表，增加 `is_system` 标记。
...
> **设计说明**：
> - `project_id` 设置 `UNIQUE` 约束，确保每项目仅保留一条偏离记录。再次偏离时执行 UPSERT（REPLACE 或 DELETE+INSERT）。
> - `impact_scope_json` 存储新增/移除/绑定变更/冻结 Stage 的完整列表，便于追溯。
...
| UT-004 | AC-09 | Given Active 项目已执行 Stage "需求探索"，When 切换模板，Then 该 Stage 状态变为 FROZEN，新模板 Stage 插入 DEFINED | 内存中构造已执行 Stage 列表 |
| UT-005 | AC-10 | Given 已执行 Stage，When 请求修改 skip_flag，Then 抛出 `STAGE_ALREADY_EXECUTED` | 内存中构造 EXECUTED 状态 |
