...
> **版本**：v1.0
> **设计状态**：FROZEN
> **上游追溯**：DR-013 详细需求（REQ-P1-001/002/003, US-006）
> **下游消费**：DR-014（监控看板——瓶颈数据消费）
> **变更**：sdlc-visualizer
...
| DR-013 | DR-005 | 产物版本历史（文件名、版本数、修改时间） | REST |
| DR-013 | DR-008 | Skill 调用日志（Skill 名称、触发时间、执行时长、状态） | REST |
...
### 3.2 `stage_executions` — Stage 执行历史表
...
### 3.3 `history_export_records` — 导出记录表
...
> 历史回溯模块**仅消费** Completed 和 Archived 状态的项目数据，
> Active 和 Draft 状态的项目不参与任何统计计算（BR-022）。
...
> 每次进入 `SkillRetry`、`GateRejected`、`ArtifactConflict` 状态时，
> 系统向 `rework_events` 表写入一条记录，作为热力图统计的数据源。
