...
| 2026-06-17 | v3.0 | DR-024 前端布局重构 v3.0：路由重组、合并页面、旧路由重定向 | AI Agent |
...
| 模块编号 | 模块名称 | 目录 | 优先级 | 设计状态 | 版本 | 上游追溯 | 下游消费 |
|----------|----------|------|--------|----------|------|----------|----------|
| DR-024 | 前端布局重构 v3.0 | `layout-v3-refactor/` | P0 | ✅ FROZEN | v1.0 | `sdlc-visualizer-menu-design-v3.md` | 全部前端页面 |

### 追加批次说明

- **性质**：纯前端重构，后端零改动
- **核心变更**：7 个一级菜单 → 4 个一级菜单（室）+ 平台管理；合并概要/详细需求、概要/详细设计、架构治理/审批；30+ 旧路由重定向
- **新增页面**：6 个合并页（脑暴室、需求方案、设计方案、设计定稿、任务编排、测试调试）
- **接口兼容性**：100% 复用现有后端 API（154 端点无变化）
- **状态管理**：复用现有 Zustand Store（`useRequirementStudioStore` / `useExecutionStore` / `useGateCenterStore`）
...
| NEXT-002 | 最终 Cross-Module Audit | 全量 21 模块 + DR-024 一致性校验 | P0 |
...
| NEXT-005 | DR-024 编码实施 | 前端路由重构、合并页面实现、旧路由重定向 | P0 |

---

## 15. Interface Contracts 产出
