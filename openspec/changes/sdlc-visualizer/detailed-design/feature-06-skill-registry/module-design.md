...
> **模块编号**：DR-006  
> **模块名称**：Skill 注册与 DAG 管理（Skill Registry & DAG Management）  
> **版本**：v1.0  
> **状态**：FROZEN  
> **设计日期**：2026-06-02  
> **上游基线**：PRD-000 v2.0-patch2 / HLD-001~003 / DR-006 详细需求
...
> **设计说明**：
> - `tags` 和 `platforms` 使用 JSON 文本序列化存储，SQLite 无原生数组类型；PostgreSQL 迁移后可改为 `JSONB`。
> - `directory_path` 用于快速定位 Skill 文件，支持"查看 SKILL.md"功能。
> - 导入时的临时状态（SCANNING / CONFLICT / SKIPPED）不持久化，仅在扫描会话内存中维护。
...
> **设计说明**：
> - `before_snapshot` / `after_snapshot` 存储 JSON 文本，记录操作前后的完整状态，支持审计和撤销。
> - 保存成功后，清空该 session 的变更日志（或归档到历史表）。MVP 阶段直接删除，P1 可考虑保留历史。
...
| HLD-003 算法 C：DAG 拓扑排序 | `DAGEditorService.save_dag()` 使用 DFS 环检测 | ✅ |
