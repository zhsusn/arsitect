...
>
> **修改记录**：v2.0 基于 brainstorming v2.0 重构：更新模板弱关联策略、产物 Git 快照、C4 混合生成模式；合并 US-001（增加模板选择）和 US-004（增加产物编辑写回）；调整业务规则与 RTM。v2.0-patch1：恢复审查功能（US-009 扩展为 P0 + 新增 US-014 + REVIEW_PENDING/REVISION_REQUESTED + REQ-P0-033~039）；恢复 C4 L3/L4 自动生成（REQ-P0-019/020 扩展）+ 反向代码定位（REQ-P0-033）；补回业务规则 BR-020~028；更新 RTM 与模块映射。
...
> 日期：2026-05-31
>
> **修改记录**：v1.3 扩展 MVP 范围，新增 US-011~US-013 及 REQ-P0-025~029、REQ-P1-005~006、BR-024~025、DR-011~DR-012。v1.2 修复 doc-quality-gate 问题：补充 US-009/US-010、修正 Stage 绑定为 1-n。v1.1 补充 US-007/US-008 及 REQ-P0-016/REQ-P0-017、BR-011~BR-014。
...
### US-003：项目管理者（路径管理与进度追踪）

**背景**：Cindy 是 PM，需要根据需求复杂度合理估算工期并跟踪进度。

**用户故事**：
> 作为 PM，当我查看项目进度时，我希望看到每个需求所处的复杂度路径和阶段状态，以便及时发现阻塞并调整资源。
