...
class ComplexityRouter:
    ...
    ) -> dict[str, ComplexityRoute]:
        """Score each dimension independently."""
        return {
            "code_lines": self._route_for_metric("code_lines", metrics.code_lines),
            "external_deps": self._route_for_metric(
                "external_deps", metrics.external_deps
            ),
            "data_models": self._route_for_metric(
                "data_models", metrics.data_models
            ),
            "api_endpoints": self._route_for_metric(
                "api_endpoints", metrics.api_endpoints
            ),
            "business_rules": self._route_for_metric(
                "business_rules", metrics.business_rules
            ),
        }
    ...
    def _build_reasoning(
        ...
        lines.append(
            f"  Code Lines: {metrics.code_lines} ({scores['code_lines'].value})"
        )
        lines.append(
            f"  External Deps: {metrics.external_deps} "
            f"({scores['external_deps'].value})"
        )
        lines.append(
            f"  Data Models: {metrics.data_models} ({scores['data_models'].value})"
        )
        lines.append(
            f"  API Endpoints: {metrics.api_endpoints} "
            f"({scores['api_endpoints'].value})"
        )
        lines.append(
            f"  Business Rules: {metrics.business_rules} "
            f"({scores['business_rules'].value})"
        )
        return "\n".join(lines)
